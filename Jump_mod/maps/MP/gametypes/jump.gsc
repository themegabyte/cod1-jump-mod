main()
{
	...
	if(getcvar("scr_jump_timelimit") == "")		// Time limit per map
		setcvar("scr_jump_timelimit", "30");
	else if(getcvarfloat("scr_jump_timelimit") > 1440)
		setcvar("scr_jump_timelimit", "1440");
	level.timelimit = getcvarfloat("scr_jump_timelimit");

	if(getcvar("scr_jump_scorelimit") == "")		// Score limit per map
		setcvar("scr_jump_scorelimit", "0");
...
Callback_StartGameType()
{
	...
	precachestring(&"Saved positions: ");
	precachestring(&"Loaded positions: ");
...
Callback_PlayerConnect()
{
	...
	self thread maps\mp\gametypes\_saveposition::initializate(); //Vitis
	self thread maps\mp\gametypes\_saveposition::doHUDMessages();
	...
	if(isdefined(self.pers["team"]) && self.pers["team"] != "spectator")
	{
		...
		{
			self notify("end_saveposition_threads"); //Vitis
...
//	self updateDeathArray();
	...
	//self dropHealth(); //Vitis

	//body = self cloneplayer(); //Vitis
...
spawnPlayer()
{
	...
	self thread maps\mp\gametypes\_saveposition::_MeleeKey(); //Vitis
	self thread maps\mp\gametypes\_saveposition::_UseKey(); //Vitis
	self thread maps\mp\gametypes\_saveposition::_UseKey2(); //Vitis
	self thread maps\mp\gametypes\_saveposition::weaponswitcher(); //Vitis
	
	self notify("welcome");	//Vitis
	if(isdefined(self.pistol)){
	self setWeaponSlotWeapon("pistol",self.pistol);
	self giveMaxAmmo(self.pistol);
	}else{
	self setWeaponSlotWeapon("pistol","luger_mp");
